...
static float lr_eval_loss_one(const __global float* normalizedParams, __global float* taskScratch)
{
	...
	float lastBlockEnergyL = 0;
	float lastBlockEnergyR = 0;
	const int lastBlockLen = 8192;
	for (int i = LR_AUTO_FFT_SIZE - lastBlockLen; i < LR_AUTO_FFT_SIZE; ++i)
	{
		lastBlockEnergyL += bufrel[i] * bufrel[i];
		lastBlockEnergyR += bufrer[i] * bufrer[i];
	}
	...
	float maxcorrloss = maxcorrl + maxcorrr + fmax(maxcorrl, maxcorrr) * 4.0f;//nice
	float lastBlockDiffLoss = fabs(lastBlockEnergyL - lastBlockEnergyR);

	lastBlockDiffLoss = lastBlockDiffLoss * 100000000.0;
